...
game::sim::ConsoleApplication::help()
{
    ...
    out.writeLine(Format(tx("PCC2 Battle Simulation Utility v%s - (c) 2020-2025 Stefan Reuther").c_str(), PCC2_VERSION));
    out.writeLine();
...
game::sim::ConsoleApplication::loadSession(Session& session, const Parameters& params, afl::charset::Charset& charset)
{
    if (session.root.get() == 0) {
        ...
        // Load root
        Ref<const game::config::UserConfiguration> uc = game::config::UserConfiguration::create();
