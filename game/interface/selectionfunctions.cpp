...
namespace {
    ...
    void mergeSelections(Selections& out, size_t outLayer,
                         const Selections& in, size_t inLayer,
                         bool merge, Selections::Kind kind)
    {
    ...
    int getSelectionMask(Selections& sel, Selections::Kind k, game::Id_t id, int layer)
    {
        if (layer < 0) {
            // All layers
            int result = 0;
            for (size_t i = 0, n = sel.getNumLayers(); i < n; ++i) {
                if (SelectionVector* p = sel.get(k, i)) {
                    if (p->get(id)) {
                        result |= 1 << i;
                    }
                }
            ...
            // One layer
            if (SelectionVector* p = sel.get(k, static_cast<size_t>(layer))) {
                return p->get(id);
            } else {
                return 0;
            }
