...
game::vcr::flak::VisualisationState::VisualisationState()
    : m_objects(), m_ships(), m_fleets(), m_smoke(), m_beams(),
      m_rng(0),
      m_maxBeamAge(5), m_maxSmokeAge(10)
{ }
...
void
game::vcr::flak::VisualisationState::createShip(Ship_t shipNr, const Position& pos, int player, bool isPlanet)
{
    // ex ACTION_CALLOUTS.snew
    Ship& sh = makeSlot(m_ships, shipNr);

    sh.player   = player;
    sh.isPlanet = isPlanet;
    sh.isAlive  = true;
