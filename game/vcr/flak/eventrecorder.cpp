    ...
    while (it.readInstruction(insn)) {
        switch (Command(insn)) {
         ...
         case cCreateShip: {
            ...
            int player;
            bool isPlanet;
            if (unpackIndex(it, shipNr) && unpackPosition(it, pos) && it.readParameter(player) && unpackBool(it, isPlanet)) {
                vis.createShip(shipNr, pos, player, isPlanet);
            }
            break;
         }
         case cKillShip: {
...
void
game::vcr::flak::EventRecorder::createShip(Ship_t shipNr, const Position& pos, int player, bool isPlanet)
{
    m_content.addInstruction(cCreateShip)
        .addParameter(packIndex(shipNr))
        .addParameter(pos.x).addParameter(pos.y).addParameter(pos.z)
        .addParameter(player)
        .addParameter(isPlanet);
}
