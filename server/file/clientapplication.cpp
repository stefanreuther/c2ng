    ...
    while (cmdl.getNext(opt, p)) {
        if (opt) {
            ...
            if (in == 0) {
                ...
                errorExit(Format(tx("too many directory names specified. Use '%s -h' for help."), environment().getInvocationName()));
            }
        }
    }

    if (out == 0) {
        errorExit(Format(tx("need two directory names (source, destination). Use '%s -h' for help."), environment().getInvocationName()));
    }
...
server::file::ClientApplication::doList(afl::sys::CommandLineParser& cmdl)
{
    ...
    if (args.empty()) {
        errorExit(Format(tx("missing directory name to list. Use '%s -h' for help."), environment().getInvocationName()));
    }
...
server::file::ClientApplication::doList(ReadOnlyDirectoryHandler& in, String_t name, bool recursive, bool longFormat, bool withHeader)
{
    ...
    for (size_t i = 0, n = children.size(); i < n; ++i) {
        ...
        if (longFormat) {
            ...
            out.writeLine(Format("%-4s %-40s %10s  %s", type, ch.contentId.orElse("-"), size, ch.name));
        } else {
...
void
server::file::ClientApplication::doCat(afl::sys::CommandLineParser& cmdl)
{
    Translator& tx = translator();
    std::vector<String_t> backends;
    std::vector<String_t> children;
    String_t p;
    bool opt;
    while (cmdl.getNext(opt, p)) {
        if (opt) {
            ...
            String_t backendPath, child;
            if (DirectoryHandlerFactory::splitPathName(p, backendPath, child)) {
                backends.push_back(backendPath);
                children.push_back(child);
            } else {
                errorExit(Format(tx("invalid file path '%s'"), p));
            }
        }
    }

    if (backends.empty()) {
        errorExit(Format(tx("missing file name to show. Use '%s -h' for help."), environment().getInvocationName()));
    }

    DirectoryHandlerFactory dhf(fileSystem(), networkStack());
    for (size_t i = 0, n = backends.size(); i < n; ++i) {
        Ref<FileMapping> map = dhf.createDirectoryHandler(backends[i], log()).getFileByName(children[i]);
        environment().attachStream(afl::sys::Environment::Output)
            ->fullWrite(map->get());
    }
}
...
server::file::ClientApplication::doClear(afl::sys::CommandLineParser& cmdl)
{
    ...
    if (args.empty()) {
        errorExit(Format(tx("missing directory name to clear. Use '%s -h' for help."), environment().getInvocationName()));
    }
...
server::file::ClientApplication::doServe(afl::sys::CommandLineParser& cmdl)
{
    // Parse parameters
    Translator& tx = translator();
    Optional<String_t> source;
    Optional<String_t> address;
    ...
    while (cmdl.getNext(opt, p)) {
        if (opt) {
            ...
            errorExit(Format(tx("too many parameters. Use '%s -h' for help."), environment().getInvocationName()));
        }
    }
    ...
    if (pSource == 0 || pAddress == 0) {
        errorExit(Format(tx("too few parameters. Use '%s -h' for help."), environment().getInvocationName()));
    }
...
server::file::ClientApplication::doGC(afl::sys::CommandLineParser& cmdl)
{
    ...
    while (gc.checkObject()) {
        if (++n % 512 == 0) {
            standardOutput().writeLine(Format("... to check: %d, reachable: %d", gc.getNumObjectsToCheck(), gc.getNumObjectsToKeep()));
            standardOutput().flush();
        }
    }
    standardOutput().writeLine(Format("Total reachable objects: %d", gc.getNumObjectsToKeep()));
    standardOutput().flush();
#ifdef MALLOC_STATS
    int postMem = mallinfo().arena;
    standardOutput().writeLine(Format("malloc: %d to %d = %d [%dk]", prevMem, postMem, (postMem-prevMem), (postMem-prevMem) / 1024));
    standardOutput().flush();
#endif
    if (gc.getNumErrors() != 0 && !force) {
        errorOutput().writeLine(Format("%d error%!1{s%} found, aborted (use \"-f\" to force)", gc.getNumErrors()));
        exit(1);
    }
    ...
    if (!dryRun) {
        while (gc.removeGarbageObjects())
            ;
        standardOutput().writeLine(Format("Total objects removed: %d", gc.getNumObjectsRemoved()));
    }
}
...
server::file::ClientApplication::doSnapshot(afl::sys::CommandLineParser& cmdl)
{
    ...
    if (args[1] == "ls") {
        ...
        for (size_t i = 0; i < list.size(); ++i) {
            if (longFormat) {
                Optional<ObjectId> objId = root.getSnapshotCommitId(list[i]);
                String_t objName = objId.isValid() ? objId.get()->toHex() : "-";
                standardOutput().writeLine(Format("%-4s %-40s %10s  %s", "SNAP", objName, "-", list[i]));
            } else {
        ...
        ObjectId objId = resolveObjectId(root, args[2]);
        ...
        errorExit(Format(tx("invalid command '%s'. Use '%s -h' for help."), args[1], environment().getInvocationName()));
    }
}
...
server::file::ClientApplication::help()
{
    ...
    out.writeLine(Format(tx("Usage:\n"
                            ...
                            "  %$0s cat FILE...\n"
                            "                      Dump content of FILEs to standard output\n"
                            "  %$0s sync SOURCE DEST\n"
...
{
    Optional<ObjectId> objId = root.getSnapshotCommitId(name);
    ObjectId* p = objId.get();
    if (p == 0) {
        errorExit(Format(translator()("unable to resolve snapshot Id '%s'"), name));
    }
    return *p;
}
