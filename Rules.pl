...
# Testsuite
if ($V{CXXTESTDIR} ne '') {
    my @ts_src = sort grep {! m:(^|/)testsuite.cpp$:} to_prefix_list($IN, $settings->{FILES_testsuite});
    ...
    # - compile & run
    # compile_executable('testsuite', [@ts_obj, @ts_main_obj, 'redir.o'], ['afl'], {LDFLAGS => '-Wl,--allow-multiple-definition -static-libstdc++'});
