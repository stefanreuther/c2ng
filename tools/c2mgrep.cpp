/**
  *  \file tools/c2mgrep.cpp
  *  \brief c2mgrep Utility - Message Search
  *
...
namespace {
    ...
    void searchVPA(Message& m, afl::io::Stream& s)
    {
        // Verify signature
        ...
        while (s.read(afl::base::fromObject(turnHeader)) == sizeof(turnHeader)) {
            if (turnHeader.signature != gt::VPA_TURN_MAGIC) {
                throw afl::except::FileFormatException(s, _("Invalid file block"));
            }
    ...
    class ConsoleMgrepApplication : public util::Application {
     public:
        ConsoleMgrepApplication(afl::sys::Environment& env, afl::io::FileSystem& fs)
            : Application(env, fs)
            { }
...
void
ConsoleMgrepApplication::appMain()
{
    // Arguments
    ...
    if (!hadSearchString) {
        errorExit(Format(_("no search string specified. Use '%s -h' for help").c_str(), environment().getInvocationName()));
    }
    if (!hadFiles) {
        errorExit(Format(_("no file name specified. Use '%s -h' for help").c_str(), environment().getInvocationName()));
    }
}


void
ConsoleMgrepApplication::searchZip(afl::io::Stream& file, String_t fname, const Job& job)
{
...
void
ConsoleMgrepApplication::searchStream(afl::io::Stream& file, const String_t& fname, const Job& job)
{
    ...
    switch (type) {
     ...
     case ZipArchive:
        if (job.optAllowZip) {
            ...
            errorOutput().writeLine(Format(_("%s: compressed file").c_str(), fname));
        }
        break;
     case UnknownFile:
        if (job.optWarnUnknown) {
            errorOutput().writeLine(Format(_("%s: unknown file format").c_str(), fname));
        }
        break;
    }
}
...
void
ConsoleMgrepApplication::searchFile(const String_t& fname, const Job& job)
{
    try {
...
ConsoleMgrepApplication::help()
{
    ...
    out.writeLine(Format(_("PCC2 Message Search v%s - (c) 2011-2018 Stefan Reuther").c_str(), PCC2_VERSION));
    out.writeLine();
...
int main(int, char** argv)
{
    ...
    return ConsoleMgrepApplication(env, fs).run();
}
