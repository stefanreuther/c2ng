/**
  *  \file u/t_server_file_ca_root.cpp
...
void
TestServerFileCaRoot::testEmpty()
{
    // Storage
    ...
    // Retrieve files
    {
        ...
        // Directory
        TS_ASSERT(rootItem.findDirectory("d") != 0);

        // File
        server::file::FileItem* fi = rootItem.findFile("f");
        TS_ASSERT(fi != 0);
        TS_ASSERT(rootItem.getFileContent(*fi)->get().equalContent(afl::string::toBytes("text")));
    }
}

/** Test operation with a preloaded image. */
void
TestServerFileCaRoot::testPreload()
{
    ...
    TS_ASSERT(fi != 0);
    TS_ASSERT(rootItem.getFileContent(*fi)->get().equalContent(afl::string::toBytes("bla\n")));
    ...
    TS_ASSERT(fi != 0);
    TS_ASSERT(rootItem.getFileContent(*fi)->get().equalContent(afl::string::toBytes("blub\n")));
}

/** Test garbage cleanup. */
void
TestServerFileCaRoot::testGarbage()
{
    using server::file::DirectoryHandler;
