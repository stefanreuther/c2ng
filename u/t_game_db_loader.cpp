...
TestGameDbLoader::testLoad()
{
    ...
    // - autobuild
    {
        TS_ASSERT_EQUALS(t.universe().planets().get(100)->getAutobuildGoal(game::FactoryBuilding), 1000);
        TS_ASSERT_EQUALS(t.universe().planets().get(100)->getAutobuildSpeed(game::FactoryBuilding), 10);
    }
    ...
    // - planet property
    {
        ...
        TS_ASSERT_EQUALS(idx, interpreter::World::pp_Comment);
        afl::data::Value* p = world.planetProperties().get(489, idx);
        TS_ASSERT(p != 0);
        TS_ASSERT_EQUALS(interpreter::toString(p, false), "Amp M |T:7");
    }
    ...
    // - ship track
    {
        ...
        TS_ASSERT(t6 != 0);
        TS_ASSERT_EQUALS(t6->x.orElse(-1), 1450);
        TS_ASSERT_EQUALS(t6->y.orElse(-1), 1361);
        TS_ASSERT_EQUALS(t6->mass.orElse(-1), 121);
        TS_ASSERT_EQUALS(t6->heading.orElse(-1), 206);
        TS_ASSERT_EQUALS(t6->speed.orElse(-1), 2);

        const game::map::ShipHistoryData::Track* t3 = sh->getHistoryLocation(3);
        TS_ASSERT(t3 != 0);
        TS_ASSERT(!t3->x.isValid());
        TS_ASSERT(!t3->y.isValid());
        TS_ASSERT(!t3->mass.isValid());
        TS_ASSERT(!t3->heading.isValid());
        TS_ASSERT(!t3->speed.isValid());
    }
    ...
    // - ship history
    {
        ...
        TS_ASSERT_EQUALS(sh->getCargo(game::Element::Neutronium).orElse(-1), 96);
        TS_ASSERT_EQUALS(sh->getHistoryTimestamp(game::map::Ship::MilitaryTime), 7);
    }

    // - ship property
    {
        afl::data::NameMap::Index_t idx = world.shipPropertyNames().getIndexByName("COMMENT");
        TS_ASSERT_EQUALS(idx, interpreter::World::sp_Comment);
        afl::data::Value* p = world.shipProperties().get(12, idx);
        TS_ASSERT(p != 0);
        TS_ASSERT_EQUALS(interpreter::toString(p, false), "Fed T6 |T:6");
    }
    ...
    // Verify
    TS_ASSERT(out.getSize() >= sizeof(FILE_DATA) - 10);
    TS_ASSERT(out.getSize() <= sizeof(FILE_DATA) + 10);
}
