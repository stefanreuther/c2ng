/**
  *  \file u/t_game_vcr_flak_eventrecorder.cpp
...
namespace {
    ...
    class Tester : public game::vcr::flak::Visualizer, public afl::test::CallReceiver {
     public:
        Tester()
            : Visualizer(), CallReceiver("Tester")
            { }
        ...
        virtual void createShip(Ship_t shipNr, const Position& pos, int player, bool isPlanet)
            { checkCall(Format("createShip(%d,%s,%d,%d)", shipNr, toString(pos), player, int(isPlanet))); }
...
void
TestGameVcrFlakEventRecorder::testIt()
{
    ...
    testee.createShip(50, Position(-50000,40000,200), 10, true);
...
void
TestGameVcrFlakEventRecorder::testSwap()
{
    // Create a Recorder. Must be empty on start.
    game::vcr::flak::EventRecorder a;
    ...
    // Replay
    {
        ...
        TS_ASSERT_THROWS_NOTHING(a.replay(t));
    ...
    game::vcr::flak::EventRecorder b;
    b.swapContent(sil);
    {
        Tester t;
        t.expectCall("killShip(1)");
        TS_ASSERT_THROWS_NOTHING(b.replay(t));
        TS_ASSERT_THROWS_NOTHING(t.checkFinish());
    }
}
