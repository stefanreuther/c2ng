...
void
TestGameSimRun::testOrderHostShip()
{
    // Environment
    TestHarness h;
    game::TeamSettings team;
    ...
    // Verify result
    TS_ASSERT(h.result.battles.get() != 0);
    TS_ASSERT_EQUALS(h.result.battles->getNumBattles(), 3U);

    // 320 (lowest FCBO) vs 384 (lowest other Id)
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(0, false)->getId(), 384);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(1, false)->getId(), 320);

    // 320 (lowest FCBO) vs 489 (second-lowest other Id)
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(0, false)->getId(), 489);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(1, false)->getId(), 320);

    // 384 (lowest other Id) vs 489 (second-lowest other Id)
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(0, false)->getId(), 489);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(1, false)->getId(), 384);
}
...
/** Test host order for planet fights, Tim-Host.
    ...
    E: verify sequence of fights is the same as generated by Host. */
void
TestGameSimRun::testOrderHostPlanet()
{
    // Environment
    TestHarness h;
    game::TeamSettings team;
...
/** Test host order for ship fights, PHost.
    ...
    E: verify sequence of fights is the same as generated by Host. */
void
TestGameSimRun::testOrderPHostShip()
{
    // Environment
    TestHarness h;
    game::TeamSettings team;
    ...
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(0, false)->getId(), 320);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(1, false)->getId(), 384);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(0, false)->getRole(), game::vcr::Object::OpponentRole);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(0)->getObject(1, false)->getRole(), game::vcr::Object::AggressorRole);

    // 384 (lowest other Id, as aggressor) vs 489 (second-lowest other Id, as opponent)
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(0, false)->getId(), 489);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(1, false)->getId(), 384);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(0, false)->getRole(), game::vcr::Object::OpponentRole);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(1)->getObject(1, false)->getRole(), game::vcr::Object::AggressorRole);

    // 489 (second-lowest other Id, as aggressor) vs 320 (as opponent)
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(0, false)->getId(), 320);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(1, false)->getId(), 489);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(0, false)->getRole(), game::vcr::Object::OpponentRole);
    TS_ASSERT_EQUALS(h.result.battles->getBattle(2)->getObject(1, false)->getRole(), game::vcr::Object::AggressorRole);
}
...
/** Test host order for planet fights, PHost.
    ...
    E: verify sequence of fights is the same as generated by Host. */
void
TestGameSimRun::testOrderPHostPlanet()
{
    // Environment
    TestHarness h;
    game::TeamSettings team;
